...
class SDC_Full_Env(gym.Env):
    ...
    def __init__(
        ...
        self.prob = prob
        self.N = prob.params['N']
        self.prob.params['u0'] = u0

        self.coll = CollGaussRadau_Right(M, 0, 1)
        self.Q = self.coll.Qmat[1:, 1:]
        self.C = None
        self.lam = None
        self.u0 = np.zeros((self.M, self.N), dtype=np.complex128)
    ...
    def _compute_residual(self, u, f):

        res = np.zeros_like(u)
        for m in range(self.M):
            # new instance of dtype_u, initialize values with 0
            res[m, :] = self.dt * self.Q[m, :].dot(f[:, :])
            res[m, :] += self.u0[m] - u[m]

        return res

    def _inf_norm(self, v):
        return max([np.linalg.norm(v[m, :], np.inf) for m in range(self.M)])
    ...
    def step(self, action):
        ...
        f = np.zeros((self.M, self.N), dtype=u.dtype)
        for m in range(self.M):
            f[m, :] = self.prob.eval_f(u[m, :])
        uex = self.prob.u_exact(self.dt)
        # Start the loop
        while not done and not self.niter >= self.max_iters:
            self.niter += 1

            # build u0 + QF(u^k)-QdF(u^k)
            integral = np.zeros_like(f)
            for m in range(self.M):
                # new instance of dtype_u, initialize values with 0
                integral[m, :] = self.dt * (self.Q[m, :] - Qdmat[m, :]).dot(f[:, :])
                integral[m, :] += self.u0[m]

            # do the sweep
            for m in range(self.M):
                # build rhs, consisting of the known values from above and new values from previous nodes (at k+1)
                rhs = integral[m] + self.dt * Qdmat[m, 0: m].dot(f[0: m, :])

                # implicit solve with prefactor stemming from the diagonal of Qd
                u[m, :] = self.prob.solve_system(rhs, self.dt * Qdmat[m, m], u[m, :])
                # update function values
                f[m, :] = self.prob.eval_f(u[m, :])

            # Compute the residual and its norm
            residual = self._compute_residual(u, f)
            norm_res = self._inf_norm(residual)
            err = np.linalg.norm(uex - u[-1,:], np.inf)
